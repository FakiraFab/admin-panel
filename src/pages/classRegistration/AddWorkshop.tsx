...
import { addWorkshop } from '../../lib/api';
...
export const AddWorkshop: React.FC<AddWorkshopProps> = ({ onClose, onSuccess }) => {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(error.message || 'Failed to add workshop', 'error');
    }
  });
    ...
    <div className="fixed inset-0 bg-black  bg-opacity-50 flex items-center justify-center  z-50">
      <div className="bg-white rounded-xl shadow-lg p-6   w-full max-w-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-[#1c1c1c]">Add Workshop</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              {mutation.isPending ? 'Adding...' : 'Add Workshop'}
            </button>
          </div>
        </form>
      </div>
